 ...
 * @date 25 May 2013
 * @date 12 June 2013 Changed to version that can be checked using jpf-android
 * 
 * @version 3.0
 * 
 */
public class DatabaseInterface {
	...
	private static final String SQL_CREATE_TABLE_RSSFEEDS = "CREATE TABLE "
			+ TABLE_RSSFEED + "(" + C_ID
			+ "  INTEGER PRIMARY KEY AUTOINCREMENT, " + C_TITLE + " String, "
			+ C_DESCRIPTION + " text, " + C_LINK + " String, " + C_PUB_DATE
			+ " String," + C_LAST_TIME_INSERTED + " long" + ");";

	// sql to create rssitem table
	private static final String SQL_CREATE_TABLE_RSSITEMS = "CREATE TABLE "
			+ TABLE_RSSITEM + "(" + C_ID
			+ "  INTEGER PRIMARY KEY AUTOINCREMENT, " + C_TITLE + " String, "
			+ C_DESCRIPTION + " text, " + C_LINK + " String, " + C_PUB_DATE
			+ " long, " + C_FEED_ID + " int )"; /*
												 * + "FOREIGN KEY(" + C_FEED_ID
												 * + ") REFERENCES " +
												 * TABLE_RSSFEED + "( " + C_ID +
												 * " )" + ");";
												 */
	...
	public Cursor getRSSFeedUpdates() {
		...
		synchronized (dbLock) {
			try {
				db = this.databaseConnection.getReadableDatabase();
				c = db.query(TABLE_RSSITEM, null, null, null, null, null,
						C_PUB_DATE + " DESC");
			} catch (Exception e) {
				Log.d(TAG,
	...
	public Cursor getRSSFeeds() {
		...
		synchronized (dbLock) {
			try {
				db = this.databaseConnection.getReadableDatabase();
				c = db.query(TABLE_RSSFEED, null, null, null, null, null,
						C_PUB_DATE + " DESC");

			} catch (Exception e) {
				Log.i(TAG,
						"Error retriving data from database: " + e.toString());
			}
		}
	 ...
	 */
	public void updatefeed(int id, String title, String description,
		...
		synchronized (dbLock) {
			try {
				db = this.databaseConnection.getWritableDatabase();
				db.execSQL("update rssfeed set title='" + title
						+ "', last_time_inserted=" + lastTimeInserted
	...
	public void clearData() {
		...
		synchronized (dbLock) {
			databaseConnection.onUpgrade(
					this.databaseConnection.getReadableDatabase(), 0, 0);
		}
	}
