...
public abstract class ScriptElementContainer extends ScriptElement implements Iterable <ScriptElement> {
  ...
  public class SECIterator implements Iterator<ScriptElement>, Cloneable {
    ...
    protected SECIterator prev; // to build ad hoc stacks
    public ScriptElement next;
    protected ScriptElement current;
    ...
    public String toString() {
      return ScriptElementContainer.this.getClass().getSimpleName() + "Iterator [next=" + next + ", current="
          + current + "]";
    }

    public void rescheduleCurrent() {
      if (current != null)
        next = current;
      else
        throw new NullPointerException("Cannot reschedule current as it is null");
  ...
  protected ScriptElement firstChild;
