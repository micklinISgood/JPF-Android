...
public class PackageManagerModelTest extends TestJPF {
  ...
  // String pActivity1 =
  // "<activity android:name=\"com.example.broadcast.MainActivity\" android:label=\"@string/app_name\" />";
  // String pActivity2 =
  // "<activity android:name=\"com.example.broadcast.MainActivity2\" android:label=\"@string/app_name\" />";
  //
  ...
  public void testReceiver2() {
    if (verifyNoPropertyViolation()) {
      String pPackageName = "za.vdm.android.testapp";
      String pReceiver = "<receiver android:name=\"za.android.vdm.rssreader.receivers.NetworkReceiver\"><intent-filter><action android:name=\"android.net.conn.CONNECTIVITY_CHANGE\" /></intent-filter></receiver>";

      String s = "<manifest package=\"" + pPackageName + "\" >" + "<application>" + pReceiver
          + "</application></manifest>";
      ...
      try {
        ...
        pi = pm.getPackageInfo("", 0);
        System.out.println(pi.packageName + " " + pPackageName);
        System.out.println(pi.receivers[0].name);
        System.out.println(pm.getFilters(pi.receivers[0].name).get(0).getAction(0));
        assert pPackageName.equals(pi.packageName);
      } catch (NameNotFoundException e) {
        e.printStackTrace();
        assert false;
