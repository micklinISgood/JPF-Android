...
/**
 * Native peer of the {@link LayoutInflater}. Makes use of
 * {@link DocumentBuilder} to parse the layout files.
 * 
...
public class JPF_android_view_LayoutInflater {
	 ...
	 * named in the R.java file. Window has count 0. TODO what happens with
	 * multi-windows come into play?
	 */
	...
	public static int setup(MJIEnv env, int objref, int fileref) {
		...
		try {
			...
			log.severe("Could not setup LayoutInflator for file: " + lPath
					+ filename + ".xml");
		}
	...
	public static int getName(MJIEnv env, int objref) {
		...
		if (n != null) {
			...
			name = currentNode.getNodeName().toLowerCase() + "_"
