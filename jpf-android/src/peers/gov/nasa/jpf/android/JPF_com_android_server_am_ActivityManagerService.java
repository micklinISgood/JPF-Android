...
public class JPF_com_android_server_am_ActivityManagerService extends NativePeer {
   ...
   * Modeled methods such as startActivity() from the native side even if it is
   * started from the native massageQueue class.
   */
   ...
   */
  @MJI
  public void init0(MJIEnv env, int activityManagerRef) {
   ...
   */
  private static void startActivityNative(MJIEnv env, int intentRef, int requestCode) {
  ...
  private static void callMethod(MJIEnv env, int classRef, String methodName, int[] argsRefs) {
    ...
    DirectCallStackFrame frame = mi.createDirectCallStackFrame(ti, argsRefs.length);
  ...
  private static void finishActivityNative(MJIEnv env, int resultCode, int resultDataRef) {
   ...
   *          the new orientation
   */
  private static void changeLayout(MJIEnv env,  int orientation) {
    log.fine("changing layout to " + orientation);

    // schedule launch of activity
    String methodName = "performConfigurationChange()V";
    int[] args = {};
    ClassInfo info = env.getClassInfo(activityManagerRef);
    callMethod(env, activityManagerRef, methodName, args);
