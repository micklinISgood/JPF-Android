 ...
 * class with {@link android.support.v4.content.LocalBroadcastManager} instead
 * of the more general facilities described below. This will give you a much
 * more efficient implementation (no cross-process communication needed) and
 ...
 * {@link Context#registerReceiver Context.registerReceiver()} or statically
 * publish an implementation through the
 * {@link android.R.styleable#AndroidManifestReceiver &lt;receiver&gt;} tag in
 * your <code>AndroidManifest.xml</code>.
 * 
 * <p>
 * <em><strong>Note:</strong></em> &nbsp;&nbsp;&nbsp;If registering a receiver
 * in your {@link android.app.Activity#onResume() Activity.onResume()}
 * implementation, you should unregister it in
 * {@link android.app.Activity#onPause() Activity.onPause()}. (You won't receive
 * intents when paused, and this will cut down on unnecessary system overhead).
 * Do not unregister in
 * {@link android.app.Activity#onSaveInstanceState(android.os.Bundle)
 * Activity.onSaveInstanceState()}, because this won't be called if the user
 ...
 * broadcast are run in an undefined order, often at the same time. This is more
 * efficient, but means that receivers cannot use the result or abort APIs
 * included here.
 * <li><b>Ordered broadcasts</b> (sent with
 * {@link Context#sendOrderedBroadcast(Intent, String)
 * Context.sendOrderedBroadcast}) are delivered to one receiver at a time. As
 ...
 * Intents that are used to start Activities with {@link Context#startActivity
 * Context.startActivity()}. There is no way for a BroadcastReceiver to see or
 ...
 * foreground operation that modifies what the user is currently interacting
 * with; broadcasting an Intent is a background operation that the user is not
 * normally aware of.
 * 
 * <p>
 * The BroadcastReceiver class (when launched as a component through a
 * manifest's {@link android.R.styleable#AndroidManifestReceiver
 * &lt;receiver&gt;} * tag) is an important part of an * <a href="{@docRoot} *
 * tag) is an important part of an <a href="{@docRoot} * tag) is an important
 * part of an * <a href="{@docRoot} tag) is an important part of an <a
 * href="{@docRoot} guide/topics/fundamentals.html#lcycles">application's
 * overall lifecycle</a>.
 * </p>
 * 
 * <p>
 ...
 * <p>
 * For information about how to use this class to receive and resolve intents,
 ...
 * cross-application facility, so you must consider how other applications may
 * be able to abuse your use of them. Some things to consider are:
 * 
 * <ul>
 * <li>
 * <p>
 * The Intent namespace is global. Make sure that Intent action names and other
 * strings are written in a namespace you own, or else you may inadvertantly
 * conflict with other applications.
 * <li>
 * <p>
 * When you use
 * {@link Context#registerReceiver(BroadcastReceiver, IntentFilter)},
 * <em>any</em> application may send broadcasts to that registered receiver. You
 * can control who can send broadcasts to it through permissions described
 * below.
 * <li>
 * <p>
 ...
 * make it unavailable to them with <code>android:exported="false"</code>.
 * <li>
 * <p>
 * When you use {@link Context#sendBroadcast(Intent)} or related methods,
 * normally any other application can receive these broadcasts. You can control
 * who can receive such broadcasts through permissions described below.
 * Alternatively, starting with
 * {@link android.os.Build.VERSION_CODES#ICE_CREAM_SANDWICH}, you can also
 * safely restrict the broadcast to a single application with
 ...
 * . Only receivers who have been granted this permission (by requesting it with
 * the {@link android.R.styleable#AndroidManifestUsesPermission
 * &lt;uses-permission&gt;} tag in their <code>AndroidManifest.xml</code>) will
 ...
 * or in the static {@link android.R.styleable#AndroidManifestReceiver
 * &lt;receiver&gt;} tag in your <code>AndroidManifest.xml</code>. Only
 * broadcasters who have been granted this permission (by requesting it with the
 * {@link android.R.styleable#AndroidManifestUsesPermission
 * &lt;uses-permission&gt;} tag in their <code>AndroidManifest.xml</code>) will
 * be able to send an Intent to the receiver.
 * 
 * <p>
 ...
 * {@link #onReceive}. Once your code returns from this function, the system
 * considers the object to be finished and no longer active.
 * 
 * <p>
 * This has important repercussions to what you can do in an {@link #onReceive}
 * implementation: anything that requires asynchronous operation is not
 ...
 * 
 * <p>
 * In particular, you may <i>not</i> show a dialog or bind to a service from
 * within a BroadcastReceiver. For the former, you should instead use the
 * {@link android.app.NotificationManager} API. For the latter, you can use
 * {@link android.content.Context#startService Context.startService()} to send a
 ...
 * running the code in its {@link #onReceive} method) is considered to be a
 * foreground process and will be kept running by the system except under cases
 ...
 * {@link android.app.Service} in conjunction with a BroadcastReceiver to keep
...
public abstract class BroadcastReceiver {
   ...
   * by {@link BroadcastReceiver#goAsync() goAsync()} while in
   * {@link BroadcastReceiver#onReceive BroadcastReceiver.onReceive()}.
   ...
   * thread-safe, however you can use it from any thread if you properly sure
   * that you do not have races. Typically this means you will hand the entire
   * object to another thread, which will be solely responsible for setting any
   * results and finally calling {@link #finish()}.
   */
  public static class PendingResult {
    ...
    /** @hide */
    public PendingResult(int resultCode, String resultData, Bundle resultExtras, int type, boolean ordered,
        boolean sticky, IBinder token) {
   ...
   * then the object is no longer alive after returning from this function.</b>
   * This means you should not perform any operations that return a result to
   * you asynchronously -- in particular, for interacting with services, you
   * should use {@link Context#startService(Intent)} instead of
   * {@link Context#bindService(Intent, ServiceConnection, int)}. If you wish to
   * interact with a service that is already running, you can use
   * {@link #peekService}.
   * 
   * <p>
   * The Intent filters used in {@link android.content.Context#registerReceiver}
   * and in application manifests are <em>not</em> guaranteed to be exclusive.
   * They are hints to the operating system about how to find suitable
   * recipients. It is possible for senders to force delivery to specific
   * recipients, bypassing filter resolution. For this reason,
   * {@link #onReceive(Context, Intent) onReceive()} implementations should
   * respond only to known actions, ignoring any unexpected Intents that they
   * may receive.
   * 
   * @param context
   ...
   *         the active broadcast. The BroadcastRecord itself is no longer
   *         active;
   *         all data and other interaction must go through
   *         {@link PendingResult} APIs. The {@link PendingResult#finish
   *         PendingResult.finish()} method
   ...
   * 
   * <p class="note">
   * This method does not work with non-ordered broadcasts such as those sent
   * with {@link Context#sendBroadcast(Intent)
   ...
   * 
   * <p>
   * <strong>This method does not work with non-ordered broadcasts such as those
   * sent with {@link Context#sendBroadcast(Intent)
