...
/**
 * Base class for those who need to maintain global application state. You can provide your own implementation
 * by specifying its name in your AndroidManifest.xml's &lt;application&gt; tag, which will cause that class
 * to be instantiated for you when the process for your application/package is created.
 * 
 * <p class="note">
 * There is normally no need to subclass Application. In most situation, static singletons can provide the
 * same functionality in a more modular way. If your singleton needs a global context (for example to register
 * broadcast receivers), the function to retrieve it can be given a {@link android.content.Context} which
 * internally uses {@link android.content.Context#getApplicationContext() Context.getApplicationContext()}
...
public class Application extends ContextWrapper implements ComponentCallbacks2 {
   ...
   * Implementations should be as quick as possible (for example using lazy initialization of state) since the
   * time spent in this function directly impacts the performance of starting the first activity, service, or
   ...
   * device, where processes are removed by simply killing them; no user code (including this callback) is
  ...
  /* package */final void attach(Context context) {
   ...
   //TODO mLoadedApk = ContextImpl.getImpl(context).mPackageInfo;
  }
