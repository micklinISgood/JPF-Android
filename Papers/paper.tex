...
\alignauthor Heila van der Merwe, Brink van der Merwe and Willem Visser\\
       ...
       \email{hvdmerwe@cs.sun.ac.za, abvdm@cs.sun.ac.za, wvisser@cs.sun.ac.za }
}
...
System events use \texttt{Intent} objects to describe a the event that occurred. They include events to start an Activity or Service or a
...
state matching and backtracking features to visit each of the execution branches. JPF stores the state of the system before the it advances
...
NewContactActivity. The ListContactsActivity contains a list of the current contacts and a button to add new contacts that directs the user
to the NewContactActivity. The NewContactActivity contains two buttons: the clear button stays on the current Activity and clears its fields
and the create button starts the \texttt{ListContactsActivity}. Now let us look at figure \ref{fig:contact} containing the input sequence
for the NewContactActivity.
...
\texttt{ListContactsActivity}'s events specified in its sequence will start executing again.
...
also adapted to store the current position of each section's events for each visited Activity. If the branch returns to a previously visited
...
JPF is automatically configured to detect thrown exceptions and to stop execution. When JPF-ANDROID was run it firstly detected the
\texttt{ArithmenticException} due to the division by zero:
\begingroup
